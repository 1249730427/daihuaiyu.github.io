...
public class Interceptor implements HandlerInterceptor {
    ...
    @Autowired
    private RedisOperatorUtil redisOperato;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        boolean hasKey = redisOperato.hasKey(USERSESSIONID + userId);
        if(!hasKey){
            return returnErrorResponse(response, ApiResponse.errorTokenMsg("登陆已过期,请重新登陆!"));
        }
        if(!userToken.equalsIgnoreCase(redisOperato.get(USERSESSIONID + userId))){
    ...
    private boolean returnErrorResponse(HttpServletResponse response, ApiResponse errorMsg) throws IOException {
        ...
        try {
            ...
            printWriter.write(errorMsg.getMsg());
            printWriter.flush();
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
